...
switch (true) do {
	...
	case (_item == "lappi"): {
		...
		_wlanZones = ["wlan_1","wlan_2","wlan_3"]; {
			...
			if(_wlanZone == "") exitWith {
				hint "Du kannst den Laptop hier nicht aufbauen, da du hier kein WLAN hast!";
			};
			_table = nearestObject[player, "OfficeTable_01_new_f"]; // Class für Tisch kann getauscht werden
			_tablePos = getPos _table;
			if(player distance _tablePos > 5) exitWith {
				hint "Du musst den Laptop an einem Tisch augbauen!";
			};
			if(([false,_item,1] call life_fnc_handleInv)) then {

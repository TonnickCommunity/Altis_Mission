...
if (LIFE_SETTINGS(getNumber,"player_vitemlogging") isEqualTo 1) then {
		if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
			money_log = format [localize "STR_balu_bef_vitem",_amount,(localize _name),[_price] call life_fnc_numberText,[BANK] call life_fnc_numberText,[CASH] call life_fnc_numberText];
		} else {
			money_log = format [localize "STR_balu_log_vitem",profileName,(getPlayerUID player),_amount,(localize _name),[_price] call life_fnc_numberText,[BANK] call life_fnc_numberText,[CASH] call life_fnc_numberText];
		};
